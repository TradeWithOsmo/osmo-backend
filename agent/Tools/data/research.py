...
async def _fetch_market_snapshot(
    ...
    # 2 – Technical analysis (non-blocking, best-effort)
    try:
        ta_data = await get_technical_analysis(symbol, timeframe=timeframe, asset_type=asset_type)
    ...
    # 3 – High/Low levels
    try:
        levels = await get_high_low_levels(symbol, timeframe=timeframe, lookback=7, asset_type=asset_type)
    ...
    # 4 – Funding rate (crypto only)
    if asset_type == "crypto":
        try:
            funding = await get_funding_rate(symbol, asset_type=asset_type)
            if isinstance(funding, dict) and not funding.get("error"):
                snap.funding_rate = funding.get("rate") or funding.get("funding_rate")
        except Exception:
            pass

    # 5 – Orderbook Depth (optional)
    if include_depth and asset_type == "crypto":
        try:
            depth = await get_orderbook(symbol, asset_type=asset_type)
            if isinstance(depth, dict) and not depth.get("error"):
                # rough liquidity check
                bids = depth.get("bids", [])
                asks = depth.get("asks", [])
                if bids and asks:
                     # basic spread check from orderbook
                    best_bid = float(bids[0][0])
                    best_ask = float(asks[0][0])
                    if best_bid > 0:
                        ob_spread = (best_ask - best_bid) / best_bid * 100
                        # Store this implicitly or extend MarketSnapshot if needed
                        # For now, we'll just check if it's "healthy"
                        if ob_spread > 1.0:
